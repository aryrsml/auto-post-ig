...
export async function generateFeedImage(textId, grade = '', takhrij = '') {
  ...
  // Footer grade kecil di bawah area krem (y ~ 640) - truncate jika terlalu panjang
  let footerRaw = [grade, takhrij].filter(Boolean).join(' • ');
  if (footerRaw.length > 70) footerRaw = footerRaw.slice(0, 67).trim() + '…';
  const footer = footerRaw ? escapeXml(footerRaw) : '';
